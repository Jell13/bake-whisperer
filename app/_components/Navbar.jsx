    ...
    return (
        <>
            {isScrolling ? (
                ...
                <NavBarFixed startAnimation={startAnimation} />
            )}
        </>
    );
};
...
const NavBarFixed = ({ startAnimation }) => {
    ...
    return (
        <header
            ref={headerRef}
            className="absolute w-full flex z-50 justify-between items-center md:px-12 px-2 py-4"
        >
            ...
            <nav className="flex md:gap-4 gap-1 col-start-1 col-span-4 md:justify-start lg:pl-6 md:items-center flex-col md:flex-row pr-6 flex-wrap">
                {links.map(({ id, name, path }) => (
                    ...
                    <a
                        href={path}
                        className="md:border-[2px] group underline-none border-softer md:px-2 md:py-1 md:rounded-2xl hover:bg-softer duration-300"
                        key={id}
                    >
                        <p className="md:text-lg text-base font-Open font-medium tracking-tighter flex text-softer group-hover:text-[#FFFFFF] duration-300">
                            {name}
                        </p>
                    </a>
                ))}
...
export const NavBarScroll = () => {
    ...
    return (
        <>
            ...
            >
                <span
                    className={`w-7 h-[2px] bg-softer absolute rounded-full ${active ? "translate-y-0 rotate-45 bg-softer" : "-translate-y-1 rotate-0"} duration-300`}
                ></span>
                <span
                    className={`w-7 h-[2px] bg-softer absolute rounded-full ${active ? "translate-y-0 -rotate-45 bg-softer" : "translate-y-1 rotate-0"} duration-300`}
                ></span>
            ...
            {active && (
                <motion.div className="w-full h-screen fixed top-0 left-0 z-30 justify-end duration-300">
                            ...
                            >
                                {links.map((links) => (
                                    <motion.li
                                        ...
                                        key={links.id}
                                        className="text-[3rem] font-semibold group relative flex w-fit gap-4 cursor-pointer items-center"
                                    >
                            ...
                            >
                                {links.map((links) => (
                                    <motion.li
                                        variants={wordVariants}
                                        key={links.id}
                                        className="text-[4rem] text-soft font-semibold group relative flex w-fit gap-4 cursor-pointer items-center"
                                    >
                                        <span className="w-3 h-3 absolute invisible opacity-0 bg-soft rounded-full group-hover:visible group-hover:opacity-100 group-hover:scale-100 scale-0 duration-300"></span>
                                        <Link
                                            href={links.path}
                                            onClick={() => setActive(false)}
                                            className="group-hover:translate-x-7 duration-700 ease-out"
...
export const NavBarScrollCart = () => {
    ...
    useEffect(() => {
        if (activeCart && activeCart.items) {
            const newTotalPrice = activeCart.items.reduce(
        ...
        <>
            ...
            >
                <HiOutlineShoppingBag
                    size={30}
                    className={`${!active ? "flex" : "hidden"} text-walnut`}
                />
                <RxCross2
                    size={30}
                    className={`${active ? "flex" : "hidden"} text-walnut`}
                    ...
                    <div className="w-full h-screen flex justify-end font-Corn">
                        ...
                        >
                            {activeCart && activeCart.items.length > 0 ? (
                                <div className="relative h-full px-10 leading-tight flex flex-col mt-12 gap-8">
                                    <div className="flex flex-col gap-2">
                                        <h4 className="font-Corn text-[2rem] text-walnut font-semibold tracking-tighter">
                                            Cart
                                        </h4>
                                ...
                                <div className="w-full h-full px-10 leading-tight flex flex-col mt-12 gap-12">
                                    ...
                                    <h5 className="font-Corn text-[1.5rem] text-walnut font-medium tracking-tighter">
                                        Cart is Empty
                                    </h5>
                                </div>
                            )}
